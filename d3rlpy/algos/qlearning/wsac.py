...
__all__ = ["WSACConfig", "WSAC", "DiscreteWSACConfig", "DiscreteWSAC"]


@dataclasses.dataclass()
class WSACConfig(LearnableConfig):
    r"""Config Soft Actor-Critic algorithm.

    WSAC is a DDPG-based maximum entropy RL algorithm, which produces
    state-of-the-art performance in online RL settings.
    WSAC leverages twin Q functions proposed in TD3. Additionally,
    ...
    adaptive_update: AdaptiveUpdatePolicy = DefaultUpdatePolicy()

    def create(self, device: DeviceArg = False) -> "WSAC":
        return WSAC(self, device)
...
class WSAC(QLearningAlgoBase[WSACImpl, WSACConfig]):
    def inner_create_impl(
        ...
        self._impl = WSACImpl(
            ...
            device=self._device,
            adaptive_update=self._config.adaptive_update,
        )
...
@dataclasses.dataclass()
class DiscreteWSACConfig(LearnableConfig):
    r"""Config of Soft Actor-Critic algorithm for discrete action-space.

    This discrete version of WSAC is built based on continuous version of WSAC
    ...
    def create(self, device: DeviceArg = False) -> "DiscreteWSAC":
        return DiscreteWSAC(self, device)

    @staticmethod
    def get_type() -> str:
        return "discrete_wsac"


class DiscreteWSAC(QLearningAlgoBase[DiscreteWSACImpl, DiscreteWSACConfig]):
    def inner_create_impl(
        self, observation_shape: Shape, action_size: int
    ) -> None:
...
register_learnable(WSACConfig)
register_learnable(DiscreteWSACConfig)
