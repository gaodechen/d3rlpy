...
__all__ = ["WSACImpl", "DiscreteWSACImpl", "WSACModules", "DiscreteWSACModules"]


@dataclasses.dataclass(frozen=True)
class WSACModules(DDPGBaseModules):
    policy: NormalPolicy
    log_temp: Parameter
    temp_optim: Optional[Optimizer]


class WSACImpl(DDPGBaseImpl):
    _modules: WSACModules
    ...
    def _adaptive_update(self, batch, metrics) -> Dict[str, float]:
        if self.adaptive_update.next_to_update(metrics) == "actor":
    ...
    def inner_update(
        ...
        #  Adaptive update for actor and critic   
        metrics = self._adaptive_update(batch, metrics)
        metrics = self._adaptive_update(batch, metrics)
...
@dataclasses.dataclass(frozen=True)
class DiscreteWSACModules(Modules):
...
class DiscreteWSACImpl(DiscreteQFunctionMixin, QLearningAlgoImplBase):
    _modules: DiscreteWSACModules
    ...
    def inner_update(
        ...
        # lagrangian parameter update for WSAC temeprature
        if self._modules.temp_optim:
            metrics.update(self.update_temp(batch))
        metrics.update(self.update_critic(batch))
        metrics.update(self.update_actor(batch))
